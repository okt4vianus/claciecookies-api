...
checkoutRoute.openapi(
  createRoute({
    ...
    request: {
      body: {
        content: { "application/json": { schema: CreateCheckoutBodySchema } },
      },
    },
    responses: {
      201: {
  ...
  async (c) => {
    ...
    try {
      ...
      if (!cart || cart.items.length === 0)
        return c.json({ message: "Cart not found or empty" }, 404);
      if (!address) return c.json({ message: "Address not found" }, 404);
      if (!shippingMethod)
        return c.json({ message: "Shipping method not found" }, 404);
      if (!paymentMethod)
      ...
      if (!userParsed.success)
        return c.json(
          {
            message:
              "Please complete customer information: full name, email address, phone number",
          },
          400
        );
